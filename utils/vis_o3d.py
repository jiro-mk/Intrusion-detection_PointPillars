...
def vis_pc(pc, bboxes=None, labels=None):
    ...
    area_box = ([[50,-5,-2],[50,-5,2],[0,-5,2],[0,-5,-2],[50,0,-2],[50,0,2],[0,0,2],[0,0,-2]])
    area_color=[0.5,0.5,1]
    ...
    vis_objs = [pc]
    vis_objs.append(bbox_obj(area_box, color=area_color))
    vis_label =[]
    for i in range(len(bboxes)):
        ...
        if labels is None:
            ...
            if labels[i] >= 0 and labels[i] < 3:
                ...
                color = COLORS[-1]
        if all(in_hull(bbox,area_box)) == True:
            vis_objs.append(bbox_obj(bbox, color=color))
            vis_label.append(labels[i])
    vis_core(vis_objs)
    return vis_objs,vis_label
