...
def main(args):
    ...
    objects,vis_label = vis_pc(pc, bboxes=lidar_bboxes, labels=labels)
    ss = objects
    with open('detection_Result.csv', 'w') as f:
        print("frame_ID,time,label,3dbbox_0,3dbbox_1,3dbbox_2,3dbbox_3,3dbbox_4,3dbbox_5,3dbbox_6,3dbbox_7", file=f)
        for i in range(len(ss)-2):
            time = d
            label = vis_label[i]
            bbox3d  = np.asarray(ss[i+2].points)
            print(f"{frame_id},{time},{label},{bbox3d[0]},{bbox3d[1]},{bbox3d[2]},{bbox3d[3]},{bbox3d[4]},{bbox3d[5]},{bbox3d[6]},{bbox3d[7]}", file=f)
    frame_ID += 1
    c = collections.Counter(vis_label)
    print("frame{0}:Pedestrian:{1}, Cyclist:{2}, Car:{3}".format(frame_ID,c[0],c[1],c[2]))
